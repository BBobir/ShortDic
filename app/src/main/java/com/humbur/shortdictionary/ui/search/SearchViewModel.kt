...
class SearchViewModel(private val searchRepository: SearchRepository) : ViewModel() {
    ...
    private fun getWords() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            searchRepository.getAllData().catch {
                _allWords.postValue(Resource.error(it.message.toString(), null))
            }
                .collect { data ->
                    val list = ArrayList<Dictionary>()
                    val cursor =
                        data.rawQuery("SELECT * FROM `WordsEntity` ORDER BY `eng` ASC", null)
    ...
    fun searchWord(word: String) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            searchRepository.getAllData().catch {
                _allWords.postValue(Resource.error(it.message.toString(), null))
            }.collect { assetDatabaseOpenHelper ->
                val cursor = assetDatabaseOpenHelper.rawQuery(
                    "SELECT * FROM `WordsEntity` WHERE `eng` LIKE '%$word%' OR `shortA` LIKE '%$word%'",
                    null
                )
                while (cursor.moveToNext()) {
                    val id = cursor.getInt(0)
